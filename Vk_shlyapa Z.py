    ...
    if event.type == VkEventType.MESSAGE_NEW:
        if event.to_me:
            ...
            if (msg == 'привет') or (msg == '✋привет'):
                sender(id, 'hola amigo, ю хэв э проблем?(если хочешь полный список команд,'
                           ' пиши: дай мне всё что у тебя есть)')
            elif msg == 'дай мне всё что у тебя есть':
                sender(id, 'найди значание выражения: 9+9+4+5+0-0-1-2-32-433+233*234-12-23+13')
            elif msg == '54059':
                sender(id, 'а теперь найди из этого квадратный корень, и результат округлить до тысячных')
            elif msg == '232,505':
                sender(id, 'https://disk.yandex.ru/client/disk/Команды')
